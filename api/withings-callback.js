...
export default async function handler(req, res) {
  ...
  const storedState = req.cookies?.withings_oauth_state;

  if (req.query?.error) {
    return res.status(400).json({
      error: `Withings authorization failed: ${req.query.error}`
    });
  }

  if (!code) {
    return res.status(400).json({
      error: "Missing Withings authorization code."
    });
  }
  ...
  if (
    ...
    return res.status(400).json({
      error: "Invalid Withings OAuth state."
    });
  }
  ...
  if (!clientId || !clientSecret) {
    return res.status(500).json({
      error: "Withings server configuration is incomplete."
    });
  }
  ...
  try {
    ...
    const tokenData = tokenResult.body;
    ...
    res.setHeader("Set-Cookie", [
      createCookie(
        ...
        tokenData.access_token,
        {
          httpOnly: true,
          secure,
          maxAge: tokenData.expires_in || 10800
        }
      ),
    ...
    console.error("Unexpected Withings callback error", {
      message:
